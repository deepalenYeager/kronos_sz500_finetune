...
def fetch_csi2000_symbols() -> list[str]:
    """获取中证2000成分股"""
    cons_df = ak.index_stock_cons(symbol="932000")
...
def main():
    parser = argparse.ArgumentParser(description="Download CSI2000 daily data via AKShare.")
    parser.add_argument("--out-dir", default="csi2000_2y/csi2000_2y")
    parser.add_argument("--start-date", default="20230101", help="YYYYMMDD")
    ...
    symbols = fetch_csi2000_symbols()
    print(f"Found {len(symbols)} CSI2000 constituents")
    ...
    print(f"\nDone! Successfully downloaded {len(symbols) - len(failures)} stocks.")
